...
def transform_func(fintech_df):

    numeric_cols = fintech_df.select_dtypes(include=['number']).drop(
        columns=['grade', 'grade_encoded', 'loan_id', 'emp_length']).columns
...
def extract_clean(filename):
    ...
    cleaned_df, lookup_table = clean(df)
    # cleaned_df.to_csv('./data/fintech_clean.csv',
    #                   index=False)
    cleaned_df.to_parquet(
        './data/fintech_clean.parquet', engine='pyarrow')
...
def transform(filename):
    ...
    #     './data/fintech_transformed.csv', index=False)
    transformed_df.to_parquet(
        './data/fintech_transformed.parquet', engine='pyarrow')


def load_to_db(filename):
    # df = pd.read_csv(filename)
    df = pd.read_parquet(filename)
    save_to_db(df, 'fintech_data_MET_P1_52_16824')
